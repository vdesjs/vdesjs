...
function getLanguageServiceHost(scriptKind: ts.ScriptKind) {
  ...
  const jsLanguageService = import(/* webpackChunkName: "javascriptLibs" */ './javascriptLibs').then(libs => {
    const host: ts.LanguageServiceHost = {
      ...
      getScriptFileNames: () => [currentTextDocument.uri, 'wechat'],
      ...
      getScriptVersion: (fileName: string) => {
        ...
        return '1'; // default lib an @types/wechat-miniprogram/index.d.ts are static
      },
...
export function getJavaScriptMode(documentRegions: LanguageModelCache<WXMLDocumentRegions>, languageId: 'javascript' | 'typescript', workspace: Workspace): LanguageMode {
  ...
  return {
    ...
    async doValidation(document: TextDocument, settings = workspace.settings): Promise<Diagnostic[]> {
      ...
      console.log("getJavaScriptMode", jsDocument)
