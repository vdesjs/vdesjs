...
export interface WXMLDocumentRegions {
...
export function getDocumentRegions(languageService: LanguageService, document: TextDocument): WXMLDocumentRegions {
  ...
  while (token !== TokenType.EOS) {
    switch (token) {
      ...
      case TokenType.Script:
        console.log("getDocumentRegions TokenType.Script", scanner.getTokenOffset(), scanner.getTokenEnd())
      ...
      case TokenType.AttributeValue:
        if (lastAttributeName === 'src' && lastTagName.toLowerCase() === 'wxs') {
          ...
          // console.log("embeddedSupport", document.getText(Range.create(document.positionAt(start), document.positionAt(end))))
          // const attrVal = document.getText(Range.create(document.positionAt(start), document.positionAt(end)));
          const dataRange = getDataRange(document, start, end)
          if (dataRange != null) {
            console.log("embeddedSupport getDataRange", document.getText(Range.create(document.positionAt(dataRange.start), document.positionAt(dataRange.end))))
            regions.push({ languageId: "wxmlData", start: dataRange.start, end: dataRange.end, attributeValue: true });
          }
      ...
      case TokenType.Content:
        // console.log("TokenType.Content", scanner.getTokenText())
        const dataRange = getDataRange(document, scanner.getTokenOffset(), scanner.getTokenEnd())
        if (dataRange != null) {
          console.log("embeddedSupport getDataRange", document.getText(Range.create(document.positionAt(dataRange.start), document.positionAt(dataRange.end))))
          regions.push({ languageId: "wxmlData", start: dataRange.start, end: dataRange.end, attributeValue: true });
        }
...
function getAttributeLanguage(attributeName: string): string | null {
  // let match = attributeName.match(/^(style)$|^(bind\w+)$/i);
  // if (!match) {
  //   return null;
  // }

  if (attributeName.indexOf("bind") == 0) {
    return "jsBind"
...
function getDataRange(document: TextDocument, start: number, end: number): { start: number, end: number } | null {
